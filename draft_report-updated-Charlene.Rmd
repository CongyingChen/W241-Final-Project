...
```{r}
pwr.t2n.test(n1 = 16, n2 = 13, d = -1.883899)
```
...
```{r}
pwr.t.test(d = 1.883899, power = 0.9, 
          , alternative = "two.sided")
```

With our hypothesis that a week, that is treated by meatless on eday of week, would be different from the control week by reducing the average weekly consumption by 1/7th. Hence, this power calculation is looking at if we expect to be able to detect a treatment effect of that level, how many number of subjects does the experiment need. As a result, the experiment need 21 subjects t detect the desired treatment effect. 
```{r}
...
pwr.t.test(d = d_p, power = 0.8, 
          , alternative = "two.sided")
```
The graph above shows that, inn order to get at least a decent 80% power, and the desired treatment effect, the experiment need to have at least 21 subjects. 
